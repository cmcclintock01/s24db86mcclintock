...
router.get('/', function(req, res, next) {
  res.render('index', { title: 'Parent App', user : req.user });
});

router.get('/register', function (req, res){
  res.render('register', {title: 'Parent App Registration'});
});
    ...
    .then(function (user){
      ...
      Account.register(newAccount, req.body.password, function(err, user){
        if (err) {
          ...
          return res.rend('register', {title: 'Registration', message: 'access error', account: req.body.username})
        }
        if (!user){
...
router.get('/login', function(req, res){
  res.render('login', { title: 'Parent App Login', user: req.user});
});
